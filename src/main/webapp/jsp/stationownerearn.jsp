...
<script type="text/javascript">
    ...
    var seriesData = [];
    $.ajax({
        url:'${pageContext.request.contextPath}/stationowner/earnowner',
        dataType:'json',
        async: false,
        processType:true,
    ...
    $(function () {
        ...
        $('#selectBtn').on('click',function () {
            var date = $('#datetimepickerVal1').val();
            ...
            if (date != "") {
                    if (provinceName == '') {
                        //查询时间段内 全国 的用户来源比例
                        $.ajax({
                            url:'${pageContext.request.contextPath}/stationowner/earnowner?date='+date,
                            success: function(data) {
                                option.series[0].data = data;
                                option.title.text = date+'日前一天全国站主收益比例';
                                initChart();
                                $('#table1').bootstrapTable('refresh',
                                    {url: '<%=request.getContextPath()%>/stationowner/earnownerfort?date='+date});
                            }
                        })
                    }else {
                        if (cityName != ''){
                            //查询时间段内 市 的用户来源比例
                            $.ajax({
                                url:'${pageContext.request.contextPath}/stationowner/earnowner?date='+date+'&cityName='+cityName,
                                success: function(data) {
                                    option.series[0].data = data;
                                    option.title.text = date+'日前一天'+provinceName+cityName+'站主收益比例';
                                    initChart();
                                    $('#table1').bootstrapTable('refresh',
                                        {url: '<%=request.getContextPath()%>/stationowner/earnownerfort?date='+date
                                            +'&cityName='+cityName});
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    option.series[0].data = data;
                                    option.title.text = date+'日前一天'+provinceName+'站主收益比例';
                                    initChart();
                                    $('#table1').bootstrapTable('refresh',
                                        {url: '<%=request.getContextPath()%>/stationowner/earnownerfort?date='+date
                                            +'&provinceName='+provinceName});
                ...
                if (provinceName == "") {
                    ...
                    if (cityName == ""){
                        //查询 省
                        $.ajax({
                            url:'${pageContext.request.contextPath}/stationowner/earnowner?provinceName='+provinceName,
                            success: function(data) {
                                option.series[0].data = data;
                                option.title.text = provinceName+'昨日站主收益比例';
                                initChart();
                                $('#table1').bootstrapTable('refresh',
                                    {url: '<%=request.getContextPath()%>/stationowner/earnownerfort?provinceName='+provinceName});
                            }
                        })
                    }else {
                        // 查询 市
                        $.ajax({
                            url:'${pageContext.request.contextPath}/stationowner/earnowner?cityName='+cityName,
                            success: function(data) {
                                console.log(data)
                                option.series[0].data = data;
                                ...
                                initChart();
                                $('#table1').bootstrapTable('refresh',
                                    {url: '<%=request.getContextPath()%>/stationowner/earnownerfort?cityName='+cityName});
                            }
                        })
                    }
                }
    ...
    function initTable() {
        $("#table1").bootstrapTable({
            url: '${pageContext.request.contextPath}/stationowner/earnownerfort', // 获取表格数据的url
